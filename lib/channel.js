...
// Implementation of the cast channel protocol as a stream
...
AuthCastMessageDecoder.prototype._transform = function(chunk, enc, done) {
  if (chunk.namespace === 'urn:x-cast:com.google.cast.tp.deviceauth') {
    ...
    if (chunk.payload_type != proto.CastMessage.PayloadType.BINARY) {
      this.emit('error', new Error('authentication message is not binary'));
      this.end();
      return;
    }
    if (chunk.payload_binary === null) {
    ...
    var auth;
    try {
      auth = proto.DeviceAuthMessage.decode(chunk.payload_binary);
    } catch (err) {
      this.emit('error', err);
      this.end();
      return;
