...
function CastChannel(options) {
  ...
  this.inRStream
    ...
    .on('error', propagateError)
    .pipe(tap(tapLogger('Sent to Chromecast device')))
  ...
  this.outWStream = tlsStream
    ...
    .on('error', propagateError)
    .pipe(tap(tapLogger('Received from Chromecast device')))
...
CastChannel.prototype.send = function(data, cb, timeout) {
  ...
  if (this._currentSession !== (data.destinationId || 'default')) {
    this.write({
    ...
    this._currentSession = (data.destinationId || 'default');
  }

  // Send data
  var datum = _.cloneDeep(data);
  datum.data.requestId = id;
  this.write(datum);

  if (cb) {
    // Invoked when we get an answer
    var answer = function(message) {
      if (message.namespace === datum.namespace &&
          message.data.requestId === datum.data.requestId) {
        if (timer) {
          clearTimeout(timer);
        }
...
// Close a session
CastChannel.prototype.stop = function(id) {
  this.write({
    namespace: 'urn:x-cast:com.google.cast.tp.connection',
    data: { type: 'CLOSE' },
    destination: id
...
function AuthMessageCodec() {
  ...
  var codec = this,
      urn = 'urn:x-cast:com.google.cast.tp.deviceauth';
